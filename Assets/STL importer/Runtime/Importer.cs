...
namespace Parabox.Stl
{
	...
	public static class Importer
	{
		...
		const float Milimeter = 0.001f;
		const float Meter = 1.0f;
		const float Inch = 0.0254f;

		private static float getUnit(Unit unit)
        {
            switch (unit)
            {
				case Unit.Milimeter:
		...
		/// <returns></returns>
		public static Mesh[] Import(string path, CoordinateSpace space = CoordinateSpace.Right, UpAxis axis = UpAxis.Y, bool smooth = false, IndexFormat indexFormat = IndexFormat.UInt16, Unit unit = Unit.Meter)
		{
		...
		static IEnumerable<Facet> ImportBinary(string path, Unit unit)
		{
			Facet[] facets;
            ...
            using (FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read))
            {
                using (BinaryReader br = new BinaryReader(fs, new ASCIIEncoding()))
                {
                    ...
                    for(uint i = 0; i < facetCount; i++)
                        facets[i] = br.GetFacet(unit: unit);
                }
            }
        ...
        static Facet GetFacet(this BinaryReader binaryReader, Unit unit)
        {
			Facet facet = new Facet(
				binaryReader.GetVector3(unit: unit),	// Normal
				binaryReader.GetVector3(unit: unit),	// A
				binaryReader.GetVector3(unit: unit),	// B
				binaryReader.GetVector3(unit: unit)	// C
				);
        ...
        static Vector3 GetVector3(this BinaryReader binaryReader, Unit unit)
        {
			float scale = getUnit(unit);

			return new Vector3(
				binaryReader.ReadSingle() * scale,
				binaryReader.ReadSingle() * scale,
				binaryReader.ReadSingle() * scale);
        }
		...
		static IEnumerable<Facet> ImportAscii(string path, Unit unit)
		{
			...
			using(StreamReader sr = new StreamReader(path))
			{
				...
				while(sr.Peek() > 0 && !exit)
				{
					...
					switch(state)
					{
						...
						case FACET:
							normal = StringToVec3(line.Replace("facet normal ", ""), unit);
						break;
						...
						case VERTEX:
                            ...
                            if (vertex == 0)
								a = StringToVec3(line.Replace("vertex ", ""), unit);
							else if(vertex == 2)
								c = StringToVec3(line.Replace("vertex ", ""), unit);
                            else if (vertex == 1)
								b = StringToVec3(line.Replace("vertex ", ""), unit);
                            vertex++;
						break;
		...
		static Vector3 StringToVec3(string str, Unit unit)
		{
			...
			foreach(var face in facets)
			{
				...
				for(int i = 0; i < 3; i++)
				{
					...
					if(!map.TryGetValue(hash, out index))
					{
						if(modelCoordinateSpace == CoordinateSpace.Right)
						{
							pos.Add(Stl.ToCoordinateSpace(points[i]));
							nrm.Add(Stl.ToCoordinateSpace(smoothNormals[hash].normalized));
						}
						else
						{
							pos.Add(points[i]);
		...
		static Mesh[] ImportHardNormals(IEnumerable<Facet> faces, CoordinateSpace modelCoordinateSpace, UpAxis modelUpAxis, IndexFormat indexFormat)
		{
			...
			for(int meshIndex = 0; meshIndex < meshes.Length; meshIndex++)
			{
				...
				{
					v[i + 0] = Stl.ToCoordinateSpace(v[i + 0]);
					v[i + 1] = Stl.ToCoordinateSpace(v[i + 1]);
					v[i + 2] = Stl.ToCoordinateSpace(v[i + 2]);

					n[i + 0] = Stl.ToCoordinateSpace(n[i + 0]);
					n[i + 1] = Stl.ToCoordinateSpace(n[i + 1]);
					n[i + 2] = Stl.ToCoordinateSpace(n[i + 2]);
